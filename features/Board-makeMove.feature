Feature: Making a move in the game
  ...
  Background: An eventlistener catches click on a game board column
    Given an eventlistener is added to the html element with class board
    When eventlistener calls function makeMove on click event on that html element
    Then eventlistener function call passes a variable of type integer between 0 and 6 as argument to makeMove

  Scenario: A player makes a valid move
    Given board.playInProgress property is false upon valid move
  ...
  Scenario: A player makes a invalid move
    Given board.playInProgress property is false upon invalid move
    When there are no free positions available in selected column for more game pieces
    Then board.playInProgress property be set to false
