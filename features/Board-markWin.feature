Feature: Showing a winning combination on game board
  ...
  Scenario: The game was won by a player
    ...
    And board winCheck method called board markWin method
    Then the argument passed by board winCheck method should be a type "array" with a length of 4
    And each of the 4 elements of this array should be a type "array" with 2 elements each containing type "numbers"
    And the first of these 2 elements be set to a value between 0 to 6 and the second element be set to a value of 0 to 7
