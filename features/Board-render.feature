Feature: 4-in-a-row game graphical user interface
  ...
  Scenario: Render full game board during game
    Given upon rendering that every position on game board is filled by game pieces
  ...
  Scenario: Render half filled game board during game
    Given upon rendering that half of the positions on game board are filled with game pieces
