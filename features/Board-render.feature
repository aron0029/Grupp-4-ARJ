Feature: 4-in-a-row graphical user interface
  The game should display and update game board graphics correctly in the webbrowser

  Background: Preconditions for rendering the game board are met
    Given that a html div element with class board exists when function board.render is called
  ...
  Scenario: Game should update GUI when a valid move has been made by red player 1
    Given that function board.render was called after a move was made
    When board.currentPlayer value was 1
  ...
  Scenario: Game should update GUI when a valid move has been made by yellow player 2
    Given that function board.render was called after a move was made
    When board.currentPlayer value was 2
